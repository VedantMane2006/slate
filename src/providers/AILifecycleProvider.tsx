...
import { composeMultimodalRequest } from '../ai/composition.ts';
...
export function AILifecycleProvider({ children }: { children: React.ReactNode }) {
  ...
  const askAI = useCallback(async (objects: CanvasObject[], selection: { ids: string[] } | null) => {
    ...
    // 2. Compose payload (rendering crop, collecting fragments)
